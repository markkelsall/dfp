	...
	this.oServiceModel.read(sPath,
			{
				...
				success : function (oData, response) {
					self.successHandler(oData, oResponse, fnCallback, oAdditionalData, self);
				},
				error : function (oError) {
					self.errorHandler(oError, fnCallback, oAdditionalData, self);
				}
			}
		);
};

EAO.prototype.successHandler = function (oData, oServiceResponse, fnCallback, oAdditionalData, self) {
	var oResponse = {};
	self.requestCompletedEvent(sPath);
	oResponse.model = oData;
	oResponse.statusCode = oData.response.statusCode;
	oResponse.response = oServiceResponse;
	oResponse.additionalData = oAdditionalData;
	fnCallback.apply(oResponse, [self]);
};

EAO.prototype.errorHandler = function (oError, fnCallback, oAdditionalData, self) {
	var oResponse = {};
	self.requestCompletedEvent(sPath);
	oResponse.statusCode = oError.response.statusCode;
	oResponse.errorModel = self.messageParse(oError.response.body,null,null);
	oResponse.additionalData = oAdditionalData;
	fnCallback.apply(oResponse, [self]);
}
	...
	this.oServiceModel.update(sPath, oData,
			{
				...
				error : function (oError) {
					...
					self.sErrorMsg = oError;
					self.requestFailedEvent(that.sErrorMsg);
					oResponse.statusCode = oError.response.statusCode;
					oResponse.errorModel = self.messageParse(oError.response.body,null,null);
					fnCallback(oResponse);
				},
	...
	this.oServiceModel.remove(sPath,
			{
				...
				error : function (oError) {
					...
					oResponse.statusCode = oError.response.statusCode;
					oResponse.errorModel = self.messageParse(oError.response.body,null,null);
					oError.additionalData = oAdditionalData;
					fnCallback(oError);
				},
				async : true,
				urlParameters : null
 ...
 * @version {@link EAO}
 */
EAO.prototype.create = function(sPath, oData, fnCallback, oAdditionalData) {
	this.requestSentEvent(sPath);
	var oResponse = {};
	var that = this;
...
EAO.prototype.batchDelete = function(arrDeleteURI,fnCallback) {
	...
	this.oServiceModel.submitBatch(
		function (oData, response) {
			self.requestCompletedEvent(uri);
			oResponse.model = oData;
			oResponse.statusCode = 200;
			oResponse.response = response;
			self.oServiceModel.clearBatch();
			fnCallback(oResponse);
		},
		function (oError) {
			//self.requestCompletedEvent(uri);
